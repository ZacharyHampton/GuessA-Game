...
while not checknumber:  # While our input is not the random number
    ...
    if (checknumber == True):  # If function returns guess is equal to random number
        ...
        askForReplay = input("Would you like to play again? (Y/N) ") # Get input for wanting to play/not play again

        if askForReplay == "Y" or askForReplay == "y": # if answer equals y
          time.sleep(1) # wait 1 second
          os.system('clear') # run clear cmd
          randomnum = random.randint(1, 100) # generate new random number
          tries = 0 # set tries to 0
          checknumber = False
          usernum = input("I am thinking of a number from 1 to 100, what is it? ") # Ask for new number
          tries = tries + 1 # Increment tries
          checknumber = check(usernum) # Check input
        elif askForReplay == "N" or askForReplay == "n": # if answer equals n
          print("Okay, thanks for playing!")
          sys.exit(0) # return response code 0, and exit program
